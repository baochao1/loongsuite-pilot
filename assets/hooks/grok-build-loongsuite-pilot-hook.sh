...
# Grok Build hook entrypoint — delegates to grok-build-hook-processor.mjs.
#
...
#
# Subcommands:
#   stop / stop_failure / user_prompt_submit / session_end
#
# Fail-open 原则: 任何错误都输出 "{}" 并 exit 0,不阻塞宿主 agent。

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROCESSOR="$SCRIPT_DIR/grok-build-hook-processor.mjs"
EMPTY_RESULT='{}'
...
log_error() {
  ...
  [[ -n "$day" ]] || day="unknown"
  local dir="$data_dir/logs/grok-build/errors"
  local file="$dir/grok-build-error-$day.jsonl"
  mkdir -p "$dir" 2>/dev/null || return 0
  printf '{"time":"%s","gen_ai.agent.type":"grok-build","stage":"%s","error.type":"shell_%s","error.message":%s}\n' \
...
if [[ ! -f "$PROCESSOR" ]]; then
  echo "[grok-build-hook] processor not found: $PROCESSOR" >&2
...
if [[ -z "$NODE_BIN" ]]; then
  echo "[grok-build-hook] node >= $MIN_NODE_MAJOR not found" >&2
...
if ! "$NODE_BIN" "$PROCESSOR" "$SUBCOMMAND"; then
  echo "[grok-build-hook] processor failed (subcommand=$SUBCOMMAND)" >&2
