...
describe('QoderWorkTraceInput', () => {
  ...
  function makeInput(opts: { initializeHistory?: boolean } = {}) {
  ...
  it.each([true, false])('uses native usage without consulting intercept (cache present: %s)', async hasCache => {
    const sessionId = 'sess-native-usage';
    const turnId = 'turn-native-usage';
    const responseId = 'chatcmpl-native-usage';
    const readIntercept = vi.spyOn(interceptReader, 'readInterceptFile').mockResolvedValue({
      tokens: [{
        id: responseId,
        ts: Date.now(),
        promptTokens: 999,
      ...
      systemPrompt: { ts: Date.now(), content: 'Synthetic legacy intercept prompt' },
    });
    const request = buildHookEntry({
      'event.id': 'native-request',
    ...
    await fs.writeFile(
      path.join(hookLogDir, todayFileName()),
      [request, response].map(e => JSON.stringify(e)).join('\n') + '\n',
    );
    await writeSegments(sessionId, 'native.jsonl', [
      segModelStart(turnId, 'native-req', '2026-06-16T10:00:00.000Z'),
      segModelEnd(turnId, 'native-req', '2026-06-16T10:00:05.000Z', 'qwork-ultimate', {
        input_tokens: 120,
        output_tokens: 30,
        ...(hasCache ? { cache_read_input_tokens: 50, cache_creation_input_tokens: 10 } : {}),
      }),
    ]);
    await writeSdkLog('sdk-session', [
      sdkMessageStart(sessionId, responseId, '2026-06-16T10:00:00.000Z'),
      sdkMessageDelta(sessionId, '2026-06-16T10:00:05.000Z', 800, 90),
    ]);

    const input = makeInput();
    const entries = await startAndCollect(input);
    expect(await triggerCycle(input)).toEqual([]);
    await input.stop();

    expect(entries).toHaveLength(2);
    const enriched = entries.find(e => e['event.id'] === 'native-response')!;
    expect(enriched['gen_ai.usage.input_tokens']).toBe(120);
    expect(enriched['gen_ai.usage.output_tokens']).toBe(30);
    expect(enriched['gen_ai.usage.total_tokens']).toBe(150);
    expect(enriched['gen_ai.usage.cache_read.input_tokens']).toBe(hasCache ? 50 : undefined);
    expect(enriched['gen_ai.usage.cache_creation.input_tokens']).toBe(hasCache ? 10 : undefined);
    expect(enriched.time_unix_nano).toBe(response.time_unix_nano);
    const req = entries.find(e => e['event.id'] === 'native-request')!;
    expect(req.time_unix_nano).toBe(request.time_unix_nano);
    expect(req['gen_ai.system_instructions']).toBeUndefined();
    expect(readIntercept).not.toHaveBeenCalled();
  });
});
